...
export function createSessionsRoutes(getSyncEngine: () => SyncEngine | null): Hono<WebAppEnv> {
    ...
    app.get('/sessions/:id/codebuddy-models', async (c) => {
        const engine = requireSyncEngine(c, getSyncEngine)
        if (engine instanceof Response) {
            return engine
        }
        ...
        if (flavor !== 'codebuddy') {
            return c.json({
                success: false,
                error: 'CodeBuddy models are only available for CodeBuddy sessions'
            }, 400)
        }

        try {
            const result = await engine.listCodebuddyModelsForSession(sessionResult.sessionId)
            return c.json(result)
        } catch (error) {
            return c.json({
                success: false,
                error: error instanceof Error ? error.message : 'Failed to list CodeBuddy models'
            }, 500)
        }
    })
